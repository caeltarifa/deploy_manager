...
tiger_vnc = TigerVNCServerSetup(vnc_user=HOST_USER, vnc_display=":1", geometry="1920x1080", depth=24, password=VNC_PASSWORD)
...
panic_service = DeployService(
    shellFileSrc=script,
    serviceFileSrc=service_src)
...
wallpaper_service = DeployService(
    shellFileSrc=str(script),
    serviceFileSrc=str(service_src)
)
